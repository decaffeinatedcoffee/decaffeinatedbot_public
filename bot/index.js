          ...
          (async () => {
            ...
            if (userAMarriage) {
              ...
              if (userBMarriage) {
                ...
                setTimeout(async function(){ let c = await keyv.get(msg.author.id + user.id + "marriagetry"); if(c){msg.reply("<@" + user.id + "> ignored you" ); a.delete().then(()=> msg.delete()).catch(()=> console.log("err"))}}, 30000)
              }
            }
          })();
        }
              ...
              if (cause) {
                ...
                if (causeslice[1].match(/^(\S+)\s(.*)/)) {
                  ...
                  (async () => {
                    ...
                    msg.reply("You are not married to anyone!");
                     }
                  })();
                }
                else {
...
client.on("guildCreate", (guild) => {
  (async () => {
    ...
    var guildsnow = client.guilds.cache.size;
    const users = client.users.cache.filter(user => !user.bot)
    var usersnow = users.size;
    await keyv.set("totalguilds", guildsnow);
    await keyv.set("totalusers", usersnow);
    const updatedRoles = [];
